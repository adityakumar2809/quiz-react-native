...
const gameReducer = (state, action) => {
    switch (action.type) {
        case 'clean_game_data':
            ...
            return { index: -1, question: '', options: [], answer: '', responses: [] , timer: 0, intervalId: null};
        case 'set_timer':
        ...
        case 'game_time_out':
            ...
            for(let i=0; i<(action.payload - state.index - 1); i++){
                arr1 = [...arr1, `missing_${state.index + i + 1}`]
            }
            console.log(arr1);
            return { ...state, responses: arr1 };
        default:
            return state;
    }
};
...
const gameTimeOut = (dispatch) => {
    return ((questions_length, setflag) => {
        setflag(0);
        dispatch({ type: 'game_time_out', payload: questions_length });
        navigate('End', null);
    })
};
...
export const { Provider, Context } = createDataContext(
    gameReducer,
    { setTimer, startTimer, stopTimer, cleanGameData, getNextQuestion, updateResponses, gameTimeOut },
    {
        index: -1,
        question: '',
        options: [],
